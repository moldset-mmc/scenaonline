...
Для отдельного **временного предпросмотра** добавлен `Dockerfile.vercel`. Он запускает полный интерфейс с новой тестовой базой в `/tmp`. Данные такого просмотра могут сбрасываться; использовать его для реальных заявок нельзя. Пароль просмотра задаётся приватно при развёртывании. Этот запуск запрещён для окружения Vercel `production`.
