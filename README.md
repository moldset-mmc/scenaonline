...
Для отдельного **временного предпросмотра** добавлен `Dockerfile.vercel`. Он запускает сайт с новой тестовой базой в `/tmp`. Данные такого просмотра могут сбрасываться; использовать его для реальных заявок нельзя. Без настройки `SCENA_ADMIN_PASSWORD` доступны публичные страницы, а вход в кабинет закрыт. Пароль задаётся отдельно в приватных настройках Vercel; файлы с паролями в сборку не входят. Этот запуск запрещён для окружения Vercel `production`.
